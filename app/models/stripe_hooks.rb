class StripeHooks
  ...
  def on_customer_subscription_updated(incoming)
    ...
    Subscription.find_by_stripe_subscription_id(subscription.id).update_from_webhook(subscription)
  end
  ...
  def on_charge_succeeded(incoming)
    ...
    puts "charge: #{charge}"
    Registration.find_by_stripe_charge_id(charge.id).update_from_webhook(charge)
  end
